...
从 [GitHub Release](../../releases) 下载桌面版（macOS 为 `.app`，Windows 为解压后双击其中的 `.exe`）即进图形界面，无需安装 Python。首次打开时 macOS 需右键 →「打开」绕过未签名提示，Windows 在 SmartScreen 提示中点「仍要运行」；首次启动会被系统安全扫描拖慢十几秒，之后秒开。
